...
class _AddTaskState extends State<AddTask> {
  ...
  @override
  void initState() {
    controller.dummyEmploye.value.clear();
    calendarStartController.text = DateTime.now().toString().split(" ")[0];
    calendarEndController.text = DateTime.parse(calendarStartController.text)
        .add(Duration(days: 1))
        .toString()
        .split(" ")[0];
    if (widget.isEdit) {
                  ...
                  children: [
                    ...
                    Expanded(
                      child: AddTaskFiles(
                        ...
                        startDate:
                            DateTime.parse(calendarStartController.text ?? ''),
                      ),
                    ),
                  ],
                ),
                    ...
                    Expanded(
                      child: SizedBox(
                        ...
                        child: Obx(() => ListView.builder(
                              ...
                              itemCount: controller.dummyEmploye.value.length,
                              shrinkWrap: true,
                              physics: const BouncingScrollPhysics(),
                              scrollDirection: Axis.horizontal,
                              itemBuilder: (context, index) {
                                var data = controller.dummyEmploye.value[index];
                                return Container(
                                  padding: const EdgeInsets.symmetric(
                                      horizontal: 10),
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      InkWell(
                                        onTap: () {
                                          print("tap");
                                          controller.dummyEmploye.value
                                              .removeWhere((element) =>
                                                  element.email ==
                                                  controller.dummyEmploye
                                                      .value[index].email);
                                          setState(() {});
                                        },
                                        child: Image.asset(
        ...
        child: SizedBox(
          ...
          child: Center(
            child: SizedBox(
              width: Get.width * 0.8,
              child: Visibility(
                // visible: nameController.value.text.isNotEmpty &&
                //     descriptionController.value.text.isNotEmpty &&
                //     calendarStartController.value.text.isNotEmpty &&
                //     calendarEndController.value.text.isNotEmpty,
                child: MyButton(
                  ...
                  onPressed: () async {
                    ...
                    tasksCollection.forEach((element) {
                      newTasks.add({
                        "task": element.text,
                        "isCompleted": false,
                      });
                    ...
                    TaskModel taskModel = TaskModel(
                        ...
                        files: imageURL,
                        personWorking:
                            userController.dummyEmploye.value.length);
                    ...
                    if (widget.isEdit) {
                      ...
                      FirebaseFirestore.instance
                          ...
                          .set(taskModel.toJson());
                      for (int i = 0;
                          i < controller.dummyEmploye.value.length;
                          i++) {
                        UserModel employeModel = UserModel(
                          name: controller.dummyEmploye.value[i].name,
                          userId: controller.dummyEmploye.value[i].userId,
                          email: controller.dummyEmploye.value[i].email,
                          status: controller.dummyEmploye.value[i].status,
                          employeImage:
                              controller.dummyEmploye.value[i].employeImage,
                        );
                        FirebaseFirestore.instance
                            .collection(userController.companyType.value)
                            .doc('tasks')
                            ...
                            .doc("${time}")
                            .collection("users")
                            .doc(employeModel.userId)
                            .set(employeModel.toJson());
                      }
                    }
                    // print(calendarStartController.text);
                    // print(calendarEndController.text);
                    Get.back();
                  },
                  text: 'Create task',
                ),
                    ...
                    ? GestureDetector(
                        onTap: () => Get.dialog(
                          Column(
                            ...
                            (value) => setState(() => widget.hintText = value)),
                        child: Image.asset(
                          kCalendarIcon,
                          height: 20,
                        ),
...
class _CalendarPopUpState extends State<CalendarPopUp> {
  ...
  Widget build(BuildContext context) {
    final _calendarCarouselNoHeader = CalendarCarousel<Event>(
      ...
      childAspectRatio: 0.9,
      minSelectedDate: widget.startDate?.add(Duration(days: 1)) ??
